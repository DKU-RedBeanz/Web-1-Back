...
로컬 개발과 테스트 모두 각자의 개인 로컬 MySQL을 사용합니다. 공통 프로젝트의 첫 확인 기준은 DB 연결 성공입니다. 담당자와 제출 기한은 1팀에서 확정합니다.
...
- IDE에 관계없이 프로젝트 루트의 `.env.example`을 `.env`로 복사해 개인 값을 입력합니다.
- `.env`는 properties 형식으로 읽습니다. `export`나 값을 감싸는 따옴표를 사용하지 않습니다.
- 실행·테스트 작업 디렉터리를 프로젝트 루트로 맞춥니다.
- 개인 `.env`는 `.gitignore`의 `/.env`로 제외하고 `.env.example`에는 예시만 커밋합니다.
...
공통 DB 연결 과제에는 Entity 작성, 도메인 테이블 생성, CRUD, 실제 회원 인증 구현, AWS RDS 연동을 포함하지 않습니다. ERD와 Security 초기 설정은 별도 범위로 정합니다.

### 후속 회의

ERD 설계와 AWS RDS MySQL 연동 일정은 1팀 회의에서 확정합니다. 로컬·테스트용 개인 MySQL과 추후 팀 공유 RDS의 용도를 구분하여 운영합니다.
